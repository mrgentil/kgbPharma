    ...
    <div class="modal-body">
        ...
        <div class="mb-3">
            ...
            <input type="date" class="form-control" id="expiration" name="expiration"
                   value="{{ old('expiration', optional($medicament)->expiration ? $medicament->expiration->format('Y-m-d') : '') }}" required>
        </div>
            ...
            <select class="form-select" id="supplier_id" name="supplier_id" >
                ...
                @foreach($suppliers as $supplier)
                    <option value="{{ $supplier->id }}"
                        @selected(old('supplier_id', optional($medicament)->supplier_id ?? null) == $supplier->id)>
                        {{ $supplier->nom }}
                    </option>
                @endforeach
            </select>
